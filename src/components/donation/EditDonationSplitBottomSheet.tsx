...
//   // NEW: Track which input is focused to prevent overwriting while typing
...
//           initialInputs[cause.id] = targetPercentage.toFixed(2);
//         });
//       }
...
//     causes.forEach((cause: any) => {
//       // If this specific input is NOT focused, we sync it with the source of truth (percentages)
//       // This allows the slider to update the text, but prevents the logic from overriding user typing
...
//     }
//   }, [percentages, causes, focusedInput]);
//   // removed inputValues dependency to avoid cycles, added focusedInput
...
//         // Edge case: not enough room
//         otherCauses.forEach((cause: any) => { newPercentages[cause.id] = minPercentage; });
//         const totalForOthers = minPercentage * otherCount;
//         newPercentages[changedId] = Math.max(minPercentage, 100 - totalForOthers);
//       } else {
...
//         if (scaled < minPercentage) needsAdjustment = true;
//       });

//       if (!needsAdjustment) {
//         otherCauses.forEach((cause: any) => { newPercentages[cause.id] = scaledValues[cause.id]; });
//       } else {
...
//               newPercentages[item.cause.id] = Math.max(minPercentage, item.originalValue * newScaleFactor);
//             });
//           }
//         }

//         // Final sanity check for total = 100
//         let finalTotal = clampedNewPercentage;
//         otherCauses.forEach((cause: any) => finalTotal += newPercentages[cause.id]);

//         if (Math.abs(finalTotal - 100) > 0.01) {
//           const adjustment = 100 - finalTotal;
//           if (clampedNewPercentage + adjustment >= minPercentage) {
//             newPercentages[changedId] = clampedNewPercentage + adjustment;
//           }
//         }
//       }
//     }
...
//     // Note: We do NOT update inputValues here. The useEffect handles that for non-focused inputs.
//   };
...
//     // 1. Allow user to type whatever they want (e.g. "12.") without interference
//     setInputValues({ ...inputValues, [causeId]: value });
//   };

//   // UPDATED: Handle Input Blur (Commit the value)
...
//     // Update the visual input to be formatted
...
//       newInputValues[cause.id] = target.toFixed(2);
//     });
...
//       if (attributedCollective !== null && attributedCollective !== undefined &&
//         attributedCollective !== 'manual' && attributedCollective !== 'Manual') {
...
//   return (
//     <>
//       {/* Styles for the slider thumb and track */}
//       <style jsx global>{`
...
//           border: 2px solid currentColor; /* Uses the color of the text-color set in style */
...
//                           type="number"
//                           step="0.01"
//                           value={inputValues[cause.id] || ''}
//                           onChange={(e) => handleInputChange(cause.id, e.target.value)}
//                           onFocus={() => setFocusedInput(cause.id)} // LOCK SYNC
//                           onBlur={() => handleInputBlur(cause.id)} // UNLOCK SYNC & CALC
...
//                         style={{
//                           // This simulates the colored track filling up to the thumb
//                           background: `linear-gradient(to right, ${sliderColor} 0%, ${sliderColor} ${percentage}%, #E5E7EB ${percentage}%, #E5E7EB 100%)`,
//                           color: sliderColor // Used by the thumb border via 'currentColor'
//                         }}
//                       />
//                     </div>
//                   </div>

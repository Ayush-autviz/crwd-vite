...
export default function CollectiveStats({
  ...
  return (
    <Card className="mx-3 md:mx-4 mb-3 md:mb-4 bg-gray-50 border-none rounded-lg py-3 md:py-4">
      <CardContent className="p-0">
        <div className="grid grid-cols-3 gap-2 md:gap-4 text-center">
          <div
            onClick={() => onStatClick?.('Nonprofits')}
            className={`${onStatClick ? 'cursor-pointer hover:bg-gray-100 transition-colors rounded-lg py-1.5 md:py-2' : ''}`}
          >
          ...
          <div
            ...
            className={`${onStatClick ? 'cursor-pointer hover:bg-gray-100 transition-colors rounded-lg py-1.5 md:py-2' : ''}`}
          >
            <p className="text-xl md:text-2xl lg:text-3xl font-bold text-[#1600ff] mb-0.5 md:mb-1">
              {donationCount}
            </p>
            <p className="text-[10px] md:text-xs lg:text-sm font-semibold text-muted-foreground">
              Donations
